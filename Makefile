...
shell-ubuntu: docker-ubuntu
	docker run -it --rm -v ${CURDIR}:${CURDIR} -w ${CURDIR} godot-lv2host-ubuntu ${SHELL_COMMAND}
...
shell-mingw: docker-mingw
	docker run -it --rm -v ${CURDIR}:${CURDIR} -w ${CURDIR} godot-lv2host-mingw ${SHELL_COMMAND}
...
shell-osxcross: docker-osxcross
	docker run -it --rm -v ${CURDIR}:${CURDIR} -w ${CURDIR} godot-lv2host-osxcross ${SHELL_COMMAND}
