...
int LilvCircularBuffer<T>::read_channel(T *p_buffer, int p_frames) {
    const int read_index  = audio_buffer->input_read_index;
    const int write_index = audio_buffer->input_write_index;
    ...
    audio_buffer->input_read_index = (read_index + p_frames) % CIRCULAR_BUFFER_SIZE;
