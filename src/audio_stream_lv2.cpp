...
AudioStreamLv2::AudioStreamLv2() {
    Lv2Server::get_singleton()->connect("layout_changed", Callable(this, "on_layout_changed"));
    Lv2Server::get_singleton()->connect("lv2_ready", Callable(this, "on_lv2_ready"));
    active = false;
}
...
void AudioStreamLv2::set_active(bool p_active) {
    ...
    Lv2Instance *lv2_instance = get_instance();
...
void AudioStreamLv2::set_instance_name(const String &name) {
    lv2_name = name;
}

const String &AudioStreamLv2::get_instance_name() const {
    ...
    for (int i = 0; i < Lv2Server::get_singleton()->get_instance_count(); i++) {
        if (Lv2Server::get_singleton()->get_instance_name(i) == lv2_name) {
            return lv2_name;
        }
    }
...
void AudioStreamLv2::on_layout_changed() {
    notify_property_list_changed();
}

void AudioStreamLv2::on_lv2_ready(String p_lv2_name) {
    if (get_instance_name() == p_lv2_name) {
        Lv2Instance *lv2_instance = get_instance();
        if (lv2_instance != NULL) {
            lv2_instance->set_active(active);
        }
...
Lv2Instance *AudioStreamLv2::get_instance() {
    Lv2Server *lv2_server = (Lv2Server *)Engine::get_singleton()->get_singleton("Lv2Server");
    if (lv2_server != NULL) {
        Lv2Instance *lv2_instance = lv2_server->get_instance(get_instance_name());
        return lv2_instance;
    }
    return NULL;
}
...
void AudioStreamLv2::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_instance_name", "name"), &AudioStreamLv2::set_instance_name);
    ClassDB::bind_method(D_METHOD("get_instance_name"), &AudioStreamLv2::get_instance_name);
    ClassDB::bind_method(D_METHOD("on_layout_changed"), &AudioStreamLv2::on_layout_changed);
    ClassDB::bind_method(D_METHOD("on_lv2_ready", "lv2_name"), &AudioStreamLv2::on_lv2_ready);
}
